...
- Updated birthday aging application:
  - declines are suppressed before each boxer’s personal decline-onset age
...
### 23. Rankings/Title Consistency and Structured Fight Metadata

- Added public pro-career accessors (`organization_names`, `ranking_slots`, `division_names`) and moved world simulation to those public APIs.
- Expanded `world_simulation` rule controls in `rules/pro_career.json` and wired world sim behavior fully to config values.
- Rankings snapshots now enforce sanctioning-body champion consistency:
  - if player is organization champion, rank is normalized to `#1`
  - if player is stale `#1` but another boxer is champion, player is auto-normalized off `#1`
- Organization-champion draw outcomes now preserve `#1` rank (no demotion on title-retention draws).
- Sanctioning-body normalization now correctly respects `include_focus_org_always=false` without forced fallback inclusion.
- `FightHistoryEntry` now stores structured pro-fight sanctioning metadata:
  - `sanctioning_bodies`
  - `ranking_updates` (per-organization post-fight rank)
  - `organization_title_updates` (title transition notes)
- Save/load parsing for structured `ranking_updates` was hardened to handle malformed values safely.
...
- Added tests for organization-champion `#1` alignment, champion-draw rank retention, optional focus-org exclusion, and structured sanctioning metadata persistence.
...
- Follow-up pass validation:
  - `python3 -m py_compile boxing_game/*.py boxing_game/modules/*.py tests/*.py tools/*.py`
  - direct runtime smoke scripts for champion-rank alignment, draw retention, sanctioning-policy exclusion, and structured-history metadata

Result:
- `59 passed` test suite
- Follow-up pass: `pytest` could not be executed in this environment because `pytest` is unavailable and network access is blocked for package install.
